...
DEBUGTALK_FILE_PATH = os.path.join(BASE_DIR, "target", "debugtalk.py")
COOKIES_FILE_PATH = os.path.join(BASE_DIR, "target", "cookies.json")
CONFIG_FILE_PATH = os.path.join(BASE_DIR, "target", "config.json")
INIT_DATA_FILE_PATH = os.path.join(BASE_DIR, "target", "init_data.json")
...
def parse_debugtalk_variables_mapping():
    """ parse debugtalk file variables

    Returns: variables_mapping

    """
    with open(DEBUGTALK_FILE_PATH, "r", encoding="utf-8") as f:
        debugtalk_contents = f.readlines()
    variables_mapping = {}
    for index, item in enumerate(debugtalk_contents):
        if "=" in item:
            item = item.replace("\"", "").split("=")
            variables_mapping[item[0].strip()] = item[1].strip()
    # logger.debug(f"[parse debugtalk file variables] variables_mapping ==> {variables_mapping}")
    return variables_mapping


def parse_cookies_variables_mapping():
    """ parse cookies.json file variables

    Returns: variables_mapping
    """
    return parse_file_variable_mapping(COOKIES_FILE_PATH)


def parse_config_variable_mapping():
    """ parse config.json file variables

    Returns: variables_mapping
    """
    return parse_file_variable_mapping(CONFIG_FILE_PATH)


def parse_init_data_variable_mapping():
    """ parse init_data.json file variables

    Returns: variables_mapping
    """
    return parse_file_variable_mapping(INIT_DATA_FILE_PATH)


def parse_file_variable_mapping(file_path):
    """ parse init_data.json file variables
    ...
    variables_mapping = json.loads(config_contents)
    # logger.debug(f"[parse config variables mapping] variables_mapping ==> {variables_mapping}")
    return variables_mapping
