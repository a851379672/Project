...
from httprunner.parser import build_url, parse_data, parse_variables_mapping, parse_config_variable_mapping
...
class HttpRunner(object):
    ...
    def __parse_config(self, config: TConfig) -> NoReturn:
        ...
        config_variable_mapping = parse_config_variable_mapping()
        config.variables.update(config_variable_mapping)
    ...
    def run_testcase(self, testcase: TestCase) -> "HttpRunner":
        ...
        for step in self.__teststeps:
            ...
            step.skipif = parse_data(
                step.skipif, step.variables, self.__project_meta.functions
            )
            ...
            if step.skipif and eval(step.skipif):
                logger.debug(f"[满足条件,跳过步骤] skipif={step.skipif} | step_name={step.name}")
                continue
    ...
    def test_start(self, param: Dict = None) -> "HttpRunner":
        ...
        log_handler = logger.add(self.__log_path, level="DEBUG", rotation="1MB")
