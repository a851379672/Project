...
__CONFIG_TEMPLATE__ = jinja2.Template("""# config content
{% for item in contents %}{{ item }} = "{{ contents[item] }}"\n{% endfor %}""")

config_content_compile = re.compile(r"# config content.*", flags=re.M)
...
def ensure_file_abs_path_valid(file_abs_path: Text) -> Text:
    ...
    logger.debug(f"[ensure_file_abs_path_valid] raw_file_relative_name={raw_file_relative_name}")
...
def __ensure_testcase_module(path: Text) -> NoReturn:
    ...
    init_files = [
        ...
        os.path.join(os.path.dirname(os.path.dirname(path)), "__init__.py")
    ]
...
def convert_testcase_path(testcase_abs_path: Text) -> Tuple[Text, Text]:
    ...
    logger.debug(f"[convert_testcase_path] testcase_new_path={testcase_new_path}")
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    logger.debug("[make_testcase] {}".format(testcase["config"]["path"]))
    ...
    for teststep in teststeps:
        ...
        logger.debug("[make_testcase] {}".format(teststep["testcase"]))
    ...
    testcase_python_abs_dirname = os.path.dirname(testcase_python_abs_path)
    if not os.path.exists(testcase_python_abs_dirname):
        os.makedirs(testcase_python_abs_dirname)
...
def config_to_debugtalk_make(config_path):
    """ make config content to debugtalk file.
    ...
    """
    logger.info("start config to debugtalk make...")
    logger.debug(f"[make config to debugtalk] config_path ==> {config_path}")
    with open(config_path, "r", encoding="utf-8") as f:
        contents = yaml.safe_load(f)
    content = __CONFIG_TEMPLATE__.render(contents=contents)
    # logger.debug(f"[make config to debugtalk] content ==> {content}")

    with open("test_cases/debugtalk.py", "r", encoding="utf-8") as f:
        # 读取debugtalk.py文件中的内容 list
        debugtalk_contents = f.readlines()

    # 删除 # config content 之后的内容
    debugtalk_contents_start = find_index_from_list(debugtalk_contents, "# config content\n")
    debugtalk_contents = debugtalk_contents[:debugtalk_contents_start]
    logger.debug(f"[make config to debugtalk] debugtalk_contents ==> {debugtalk_contents}")

    # 更新 # config content 之后的内容
    content = content.split("###")
    debugtalk_contents += content

    with open("test_cases/debugtalk.py", "w", encoding="utf-8") as f:
        # 更新后的内容写入debugtalk.py文件中
        f.writelines(debugtalk_contents)


def find_index_from_list(input_data: list, item_value: str) -> int:
    """ Find index from a list by item value.
    ...
    """
    # 如果没有找到指定的值，就返回列表的末尾
    if not input_data.count(item_value) > 0:
...
def move_pytest_files_to_target(test_cases_root_dir):
    """ move pytest files to target and delete pytest files in test_cases

    :param test_cases_root_dir:
    :return:
    """
    logger.info("start move pytest files to target...")

    # 清空target目录
    target_path = os.path.join(Path(test_cases_root_dir).parent, "target")
    logger.info(f"Delete target {target_path}")
    if os.path.exists(target_path):
        shutil.rmtree(target_path)
    logger.info(f"Make directory target")
    os.mkdir("target")

    # 将用例转移到target目录
    for root, dirs, files in os.walk(test_cases_root_dir):
        for dir_tmp in dirs:
            if dir_tmp == "__pycache__":
                shutil.rmtree(os.path.join(root, dir_tmp))
        for file in files:
            if not os.path.exists(root.replace("test_cases", "target")):
                os.makedirs(root.replace("test_cases", "target"))
            if os.path.splitext(os.path.join(root, file))[1] == ".py":
                shutil.copy2(os.path.join(root, file), os.path.join(root.replace("test_cases", "target"), file))
            if os.path.splitext(os.path.join(root, file))[1] == ".py" and (
                    file not in ["debugtalk.py", "cctest1.py"]):
                os.remove(os.path.join(root, file))
            if file in ["pytest.ini"]:
                shutil.copy2(os.path.join(root, file), os.path.join(root.replace("test_cases", "target"), file))
