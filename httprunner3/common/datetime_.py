...
class DateTime(object):
    ...
    def time_stamp(self, datetime_):
        """
        时间格式转时间戳 ：2013-10-10 23:40:00 转 1381419600
        :param: datetime
        :return:
        """
        try:
    ...
    def diy_time(self, args):
        ...
        for func, value in date_.items():
            ...
            if len(str(datetime_[func.lower()])) == 1:
                # 是否补0
                func_value = '%02d' % func_value
                datetime_[func.lower()] = func_value
        if len(args) == 5:
            return f'{datetime_.get("year")}-{datetime_.get("month")}-{datetime_.get("day")} {datetime_.get("hour")}:' \
                   f'{datetime_.get("min")}'
        else:
            return f'{datetime_.get("year")}-{datetime_.get("month")}-{datetime_.get("day")} {datetime_.get("hour")}:' \
                f'{datetime_.get("min")}:{datetime_.get("sec")}'
...
if __name__ == "__main__":
    ...
    print(object_.stamp_time('1381419600'))
